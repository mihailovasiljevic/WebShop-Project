...
/**
 * Servlet implementation class DodajKomadNamestajaServlet
 */
public class DodajKomadNamestajaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DodajKomadNamestajaServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		proveriKomadNamestaja(request,response);
	}
	private synchronized void proveriKomadNamestaja(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		for(KomadNamestaja kn: knRepository.FindAll()){
			if(kn.getSifra().equals(tmpKomadNamestaja.getSifra())){
				String answer = mapper.writeValueAsString("komad_postoji");
				response.getWriter().write(answer);
				return;
			}
		}
		...
		knRepository.Save(tmpKomadNamestaja);
